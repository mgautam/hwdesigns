...
int main(void)
{
	...
	XGpio_SetDataDirection(&Gpio, CHANNEL_1, 0);
	XGpio_SetDataDirection(&Gpio, CHANNEL_2, 0);

	/* Loop forever blinking the LED */

	if (1) {
		/* Set the LED to High */
		XGpio_DiscreteWrite(&Gpio, CHANNEL_1, 0xf31000ff);//0xf8fffff1//0xf30fffff
		XGpio_DiscreteWrite(&Gpio, CHANNEL_2, 0x1);

		/* Wait a small amount of time so the LED is visible */
		for (Delay = 0; Delay < LED_DELAY; Delay++);

		/* Clear the LED bit */
		//XGpio_DiscreteClear(&Gpio, CHANNEL_1, 0);
		XGpio_DiscreteWrite(&Gpio, CHANNEL_1, 0);

		/* Wait a small amount of time so the LED is visible */
		for (Delay = 0; Delay < LED_DELAY; Delay++);
		XGpio_DiscreteWrite(&Gpio, CHANNEL_2, 0);
	}
	xil_printf("SPI Success\r\n");
